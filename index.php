        ...
        <?php
            foreach ($db as $questionTab) {
                echo "<div class='question-answer-tab'>";
                    ...
                    /* explode on every end of the line */
                    $para = explode(PHP_EOL, $questionTab["answer"]);
                    foreach ($para as $expPara) {
                        if (strlen($expPara) > 0) {
                            echo "<p>" . trim($expPara) . "</p>";
                        }
                    }
                    
                    if (strlen($questionTab["subquestion"]) > 0 && strlen($questionTab["subanswer"]) > 0) {
                        /* print the sub question */
                        echo "<h3>" . $questionTab["subquestion"] . "</h3>";
                        /* explode the sub answer */
                        $subPara = explode(PHP_EOL, $questionTab["subanswer"]);
                        foreach ($subPara as $expSubPara) {
                            if (strlen($expSubPara) > 0) {
                                echo "<p>" . trim($expSubPara) . "</p>";
                            }
                        }
                    }
